...
/// <summary>
/// TestXPatcher 是 XPatcher 的单元测试。
/// </summary>
public class TestXPatcher
{
    private class MyPatcher : XPatcher
    {
    ...
    {
        // 准备测试目录
        var testUri = $"TestXPatcher/Patch-{XTime.GetMillisecond()}/Scripts";
        ...
        {
            var remoteRootDir = XFile.PathJoin(XEnv.ProjectPath, "Temp/TestXPatcher");
            if (XFile.HasDirectory(remoteRootDir)) XFile.DeleteDirectory(remoteRootDir);

            var localRootDir = XFile.PathJoin(XEnv.LocalPath, "TestXPatcher");
    ...
    public void Speed(float currentTime, float speedTime, long lastSize, long speed, float expected, string _)
    {
        ...
        var phase = XPatcher.Phase.Download;

        patcher.CurrentTime = currentTime;
        patcher.SetSpeedTime(phase, speedTime);
        patcher.SetLastSize(phase, lastSize);
        patcher.SetSpeed(phase, speed);
    ...
    [TestCase(XPatcher.Phase.Extract, 0.6f, "解压阶段")]
    [TestCase(XPatcher.Phase.Validate, 0.7f, "验证阶段")]
    public void Progress(XPatcher.Phase phase, float expected, string _)
    {
        var patcher = new MyPatcher();
    ...
    [TestCase(XPatcher.Phase.Extract, 512, "解压阶段")]
    [TestCase(XPatcher.Phase.Validate, 256, "验证阶段")]
    public void Size(XPatcher.Phase phase, long expected, string _)
    {
        var patcher = new MyPatcher();
