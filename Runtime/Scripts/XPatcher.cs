...
namespace EFramework.Unity.Updater
{
    ...
    /// 更多信息请参考模块文档。
    /// </remarks>
    public partial class XPatcher : XUpdater.IWorker
    {
        /// <summary>
        /// Phase 是补丁处理阶段的枚举。
        /// </summary>
        ...
        public XPatcher(string assetUrl, string localUrl, string remoteUrl)
        {
        ...
        protected virtual IEnumerator Extract()
        {
            ...
            try
            {
                ...
                XLog.Info("XPatcher.Extract: start to extract <a href=\"file:///{0}\">{1}</a> into <a href=\"file:///{2}\">{3}</a>.", Path.GetFullPath(assetUrl), assetUrl, Path.GetFullPath(localRoot), localRoot);
                if (!XFile.HasDirectory(localRoot)) XFile.CreateDirectory(localRoot);
                XUpdater.Event.Notify(XUpdater.EventType.OnPatcherExtractStart, this);

                var lprogress = 0f;
                XFile.Unzip(assetUrl, localRoot, () => done = true, (err) =>
                {
                    XLog.Error("XPatcher.Extract: extract with error: {0}.", err);
                    Error = err;
                    done = true;
                }, (progress) => XLoom.RunInMain(() =>
                {
                    ...
                    {
                        lprogress = progress;
                        XUpdater.Event.Notify(XUpdater.EventType.OnPatcherExtractProgress, this);
                    }
                }));
            }
            ...
            if (string.IsNullOrEmpty(Error))
            {
                ...
                XUpdater.Event.Notify(XUpdater.EventType.OnPatcherExtractSucceeded, this);
                XLog.Info("XPatcher.Extract: finsh to extract, elapsed: {0}ms.", XTime.GetMillisecond() - time);
            }
            else
            {
                XUpdater.Event.Notify(XUpdater.EventType.OnPatcherExtractFailed, this);
                XLog.Error("XPatcher.Extract: finsh to extract with error: {0}, elapsed: {1}ms.", Error, XTime.GetMillisecond() - time);
            }
        }
        ...
        protected virtual Func<bool> Validate()
        {
            ...
            XLog.Info("XPatcher.Validate: start to validate <a href=\"file:///{0}\">{1}</a>.", Path.GetFullPath(localRoot), localRoot);
            ...
            try
            {
                if (!XFile.HasDirectory(localRoot)) XLog.Info("XPatcher.Validate: <a href=\"file:///{0}\">{1}</a> doesn't exist.", Path.GetFullPath(localRoot), localRoot);
                else
                {
                    ...
                    if (files.Count > 0)
                    {
                        ...
                        ThreadPool.QueueUserWorkItem((_) => // 避免从主线程衍生过多子线程引起的卡顿
                        {
                            ...
                            XLog.Info("XPatcher.Validate: sort {0} local file(s), elapsed: {1}ms.", total, XTime.GetMillisecond() - stime);
            ...
            XUpdater.Event.Notify(XUpdater.EventType.OnPatcherValidateStart, this);
            ...
            return new Func<bool>(() =>
            {
                if (total > 0 && record != current)
                {
                    ...
                    {
                        lprogress = progress;
                        XUpdater.Event.Notify(XUpdater.EventType.OnPatcherValidateProgress, this);
                    }
                }
                ...
                if (string.IsNullOrEmpty(Error))
                {
                    for (var i = DiffInfo.Added.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (md5s.TryGetValue(fi.Name, out var md5) && md5 == fi.MD5)
                        {
                            ...
                            XLog.Info("XPatcher.Validate: local file's md5 equals to remote: {0}.", fi.Name);
                        }
                    }
                        ...
                        if (!delete && md5 != fi.MD5) // 本地文件不存在
                        {
                            foreach (var rfi in RemoteMani.Files)
                            {
                                if (rfi.Name == fi.Name) // 远端存在该文件
                                {
                                    ...
                                    XLog.Info("XPatcher.Validate: local file's md5 doesn't equals to manifest: {0}.", rfi.Name);
                                    break;
                                }
                            }
                        }
                    ...
                    XUpdater.Event.Notify(XUpdater.EventType.OnPatcherValidateSucceeded, this);
                    XLog.Info("XPatcher.Validate: validated {0} local file(s)' {1} md5(s) by {2} task(s), elapsed: {3}ms.", total, md5s.Count, tasks, XTime.GetMillisecond() - time);
                }
                else
                {
                    XUpdater.Event.Notify(XUpdater.EventType.OnPatcherValidateFailed, this);
                    XLog.Error("XPatcher.Validate: validate with error: {0}, elapsed: {1}ms.", Error, XTime.GetMillisecond() - time);
                }
                return true;
            });
        }
        ...
        protected virtual IEnumerator Download()
        {
            ...
            XLog.Info("XPatcher.Download: start to download from <a href=\"{0}\">{1}</a> into <a href=\"file:///{2}\">{3}</a>.", remoteRoot, remoteRoot, Path.GetFullPath(localRoot), localRoot);
            if (!XFile.HasDirectory(localRoot)) XFile.CreateDirectory(localRoot);

            XUpdater.Event.Notify(XUpdater.EventType.OnPatcherDownloadStart, this);
            ...
            if (Size(Phase.Download) > 0) // 进度回档
            {
                ...
                if (progress > nprogress)
                {
                    ...
                    XLog.Info("XPatcher.Download: revert progress from {0} to {1}.", progress, nprogress);
                    XUpdater.Event.Notify(XUpdater.EventType.OnPatcherDownloadProgress, this);
                }
            }
                        ...
                        if (req.isDone || !string.IsNullOrEmpty(req.error))
                        {
                            ...
                            else
                            {
                                ...
                                if (XLog.Able(XLog.LevelType.Info))
                                {
                                    ...
                                    XLog.Info("XPatcher.Download: download {0} into {1}, elapsed {2}.", ssize, fi.Name, etime);
                                }
                    ...
                    if (string.IsNullOrEmpty(Error))
                    {
                        if (reqs.Count < threadCount && downloads.Count > 0)
                        {
                            ...
                            if (XLog.Able(XLog.LevelType.Info))
                            {
                                ...
                                XLog.Info("XPatcher.Download: download {0} from {1}.", ssize, fi.Name);
                            }
                        }
                    }
                }
                    ...
                    if (!string.IsNullOrEmpty(Error))
                    {
                        if (reqs.Count > 0) // 释放资源
                        {
                            foreach (var kvp in reqs)
                            {
                                ...
                                XLog.Info("XPatcher.Download: dispose request of <a href=\"{0}\">{1}</a>.", kvp.Value.uri, kvp.Value.uri);
            ...
            if (string.IsNullOrEmpty(Error))
            {
                ...
                XUpdater.Event.Notify(XUpdater.EventType.OnPatcherDownloadSucceeded, this);
                XLog.Info("XPatcher.Download: download {0} file(s) done, elapsed: {1}.", total, stime);
            }
            else
            {
                XUpdater.Event.Notify(XUpdater.EventType.OnPatcherDownloadFailed, this);
                XLog.Error("XPatcher.Download: download with error: {0}, elapsed: {1}.", Error, stime);
            }
        }
        ...
        protected virtual Func<bool> Cleanup()
        {
            ...
            var localRoot = Path.GetDirectoryName(localUrl);
            XLog.Info("XPatcher.Cleanup: start to cleanup deleted file(s) at <a href=\"file:///{0}\">{1}</a>.", Path.GetFullPath(localRoot), localRoot);
            var done = false;
            ThreadPool.QueueUserWorkItem((_) =>
            {
                try
                {
                    foreach (var fi in DiffInfo.Deleted)
                    {
                        ...
                        if (XFile.HasFile(fs))
                        {
                            XLog.Info("XPatcher.Cleanup: delete {0}.", fs);
                            XFile.DeleteFile(fs);
                        }
                    }
                }
                ...
                if (string.IsNullOrEmpty(Error)) XLog.Info("XPatcher.Cleanup: finsh to cleanup deleted file(s).");
                else XLog.Error("XPatcher.Cleanup: finsh to cleanup deleted file(s) with error: {0}.", Error);
            });
